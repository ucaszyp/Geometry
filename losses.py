...
class compute_loss(nn.Module):
    def __init__(self, args):
        ...
        self.depth_loss = self.forward_Depth

    def forward(self, *args):
        if self.args.task == "normal":
            _, _, normal_loss = self.loss_fn(*args)
            return normal_loss
        else:
            depth_loss = self.depth_loss(*args)
            return depth_loss


    # def forward(self, *args):
    #     depth_type = 'with_mask'
    #     valid_mask_1, valid_mask_2, normal_loss = self.loss_fn(*args)
    #     # depth_loss = self.depth_loss(valid_mask_1, valid_mask_2, depth_type, *args)
        ...
        for (pred, coord) in zip(pred_list, coord_list):
            if coord is None:
                ...
                if self.loss_type == 'UG_NLL_vMF':
                    ...
                    raise Exception
                valid_mask_2.append(valid_mask)
            
        return valid_mask_1, valid_mask_2, loss  


    def forward_Depth(self, pred_list, coord_list, gt_depth, gt_depth_mask):
        loss = 0.0
        # valid_mask1 = None
        # valid_mask2 = []
        for (pred, coord) in zip(pred_list, coord_list):
            if coord is None:
                pred = F.interpolate(pred, size=[gt_depth.size(2), gt_depth.size(3)], mode='bilinear', align_corners=True)
                pred_depth, pred_sigma = pred[:, 0:1, :, :], pred[:, 1:, :, :]
                
                valid_mask = gt_depth_mask[:, 0, :, :].float()
                valid_mask = valid_mask > 0.5
                
                pred_depth = pred_depth.squeeze(dim=1)
                pred_depth = pred_depth[valid_mask]
                gt = gt_depth.squeeze(dim=1)
                gt = gt[valid_mask]
                pred_sigma = pred_sigma[:, 0, :, :][valid_mask]
                # pred_sigma = torch.square(pred_sigma) + 1

                # loss_pixelwise = torch.log(pred_sigma) \
                #                     + torch.square((pred_depth-gt)/pred_sigma)/2
                ...
                gt_depth_ = F.grid_sample(gt_depth, coord, mode='bilinear', align_corners=True)  # (B, 3, 1, N)
                gt_depth_mask_ = F.grid_sample(gt_depth_mask.float(), coord, mode='nearest', align_corners=True) 
                gt_depth_ = gt_depth_[:, :, 0, :]  # (B, 3, N)
                gt_depth_mask_ = gt_depth_mask_[:, :, 0, :] > 0.5                 
                
                valid_mask = gt_depth_mask_[:, 0, :].float()
                valid_mask = valid_mask > 0.5
...
class aleatoric_loss(nn.Module):
    ...
    def forward(self, pred, depth_gt, mask):
        pred_depth = pred[:, 0:1, :, :]
        pred_sigma = pred[:, 1:, :, :]
        pred_depth = pred_depth[mask]
        pred_sigma = pred_sigma[mask]
        depth_gt = depth_gt[mask]
        loss_pixelwise = pred_sigma + torch.mul(torch.exp(-pred_sigma), torch.square(pred_depth - depth_gt))
        loss = torch.mean(loss_pixelwise)
        print(loss)
        return loss